...
const EcosystemClient = ({ecosystem}) => {


const [isEcosystem, setIsEcosystem] = useState(null);
  ...
  return (
    <div className='w-full grid md:grid-cols-2 gap-6   pb-10 ' >
      ...
      <h2 className='text-left text-[#4B5564] text-base pt-2 '>{item.goal}</h2>            
        ...
        <button className='text-center w-full bg-[#0F4082] cursor-pointer hover:bg-[#0a3676] rounded-2xl mb-5 py-3 text-white font-medium ' onClick={()=>setIsEcosystem(item)}>Learn More</button>   
            </div>
        ))}
        
        {isEcosystem && <Modal modal={isEcosystem} setModal={setIsEcosystem} />}
    </div>
  )
}
