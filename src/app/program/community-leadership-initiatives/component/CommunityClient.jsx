...
const CommunityClient = ({community}) => {
    {/*const Community = [
        {title: "Business Acceleration & Formalization",
         goal:"Strengthen business foundations and secure growth capital.",  
         subcomponents:[ "12-week cohorts","Supplier diversity certification", " 1:1 advisory"], 
         subtext:"Strengthen business foundations and secure growth capital through our comprehensive 12-week program", 
         components:["12-week intensive cohort program with structured curriculum","Supplier diversity certification preparation and support","Personalized 1:1 advisory sessions with industry experts","Business plan development and refinement","Financial modelling and projections guidance","Legal structure and compliance support"], 
         description: "This program is designed for entrepreneurs ready to formalize and scale their businesses. Limited spots available per cohort"},
  ...
  return (
    <div className='w-full grid md:grid-cols-2 gap-6   pb-10 ' >
      ...
      <h2 className='text-left text-[#4B5564] text-base pt-2 '>{item.goal}</h2>            
        ...
        <button className='text-center w-full bg-[#0F4082] cursor-pointer hover:bg-[#0a3676] rounded-2xl mb-5 py-3 text-white font-medium ' onClick={()=>setIsCommunity(item)}>Learn More</button>   
            </div>
        ))}
        
        {isCommunity && <Modal modal={isCommunity} setModal={setIsCommunity} />}
    </div>
  )
}
